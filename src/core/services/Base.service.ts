...
export class CoreBaseService {
    ...
    */
     ...
     * @param {*} data 
    */
    get(code?: string | number | undefined, query = null, raw = false, deleteEmpty = false) {
          ...
          T.getSubTypesList().forEach((item: any) => {
            if (modelInstance[item.object]) {
              if(Array.isArray(modelInstance[item.object])) {
                modelInstance[item.object].forEach((z_item: any, z_index: any) => {
                  if(item.test) {
    ...
    */
    getDeletePath(data: any, idSlug = true) {
      if (!data || !idSlug) {
        return `${this.path}`;
      }
